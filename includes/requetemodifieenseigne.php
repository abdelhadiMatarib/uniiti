    ...
    switch ($_POST['step']) {
        ...
        case "MotsCles" :
            foreach ($MotCle as $id_type_info => $value) {
                ...
                if (($MotCle[$id_type_info][1] != '') || ($MotCle[$id_type_info][2] != '') || ($MotCle[$id_type_info][3] != '')) {
                    $ACreerOuModifier = true;
                }
        ...
        case "Prestations" :
            ...
            $reqcheck = $bdd->prepare($sqlcheck);
            //$reqcheck->bindParam('id_enseigne', $id_enseigne, PDO::PARAM_INT);
            $reqcheck->execute(array('id_enseigne' => $id_enseigne, 'id_type_info' => '7'));
            $prestations = $reqcheck->fetchAll();
            
            if (!empty($_POST['prestation'])) {
                // 1) delete all previous prestations
                if (!empty($prestations)) {
                    foreach ($prestations as $key => $p) {
                        $req = $bdd->prepare("DELETE FROM enseignes_prestations_contenus WHERE id_prestation=:id_prestation");
                        $req->execute(array('id_prestation' => $p['id_enseignes_prestations']));
                    }
                }
                $req = $bdd->prepare("DELETE FROM enseignes_prestations WHERE enseignes_id_enseigne=:enseignes_id_enseigne AND id_type_info=7");
                $req->execute(array('enseignes_id_enseigne' => $id_enseigne));
                //2) insertion des nouvelles prestations
                
                foreach ($_POST['prestation'] as $presta){
                   // print_r($presta)
                     $req = $bdd->prepare("INSERT INTO enseignes_prestations (enseignes_id_enseigne, id_type_info, prestation) VALUES (:enseigne_id, 7, :prestation) ");
                     $req->execute(array('enseigne_id'=>$id_enseigne, "prestation"=>$presta['nom']));
                     $lastId = $bdd->lastInsertId();
                     foreach ($presta['ligne'] as $content){
                         $req = $bdd->prepare("INSERT INTO enseignes_prestations_contenus (id_prestation, contenu, prix) VALUES (:id_prestation, :contenu, :prix) ");
                            $req->execute(array('id_prestation'=>$lastId, "contenu"=>$content['content'], 'prix'=>$content['price']));
                     }
                }
            }
