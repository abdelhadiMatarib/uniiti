<?php
		...
		$sqldroite = " FROM ( SELECT 'avis' AS provenance, date_avis, id_avis, id_statut, 'objet' AS type, contributeurs_id_contributeur, objets_id_objet
					...
					ON t1.id_avis = t2.avis_id_avis
						...
						WHERE id_statut = 2
				UNION
					SELECT 'aime' AS provenance, date_aime AS date_avis, '' AS id_avis, '2' AS id_statut, 'objet' AS type, contributeurs_id_contributeur, objets_id_objet
					FROM contributeurs_aiment_objets AS t4
				UNION
					SELECT 'aime_pas' as provenance, date_aime_pas AS date_avis, '' AS id_avis, '2' AS id_statut, 'objet' AS type, contributeurs_id_contributeur, objets_id_objet
					FROM contributeurs_aiment_pas_objets AS t5
				UNION
					SELECT 'wish' as provenance, date_wish AS date_avis, '' AS id_avis, '2' AS id_statut, 'objet' AS type, contributeurs_id_contributeur, objets_id_objet
		...
		switch ($PAGE) {
			case "Objet" :
				$sqldroite .= " WHERE objets_id_objet = " . $id_objet;
				$sql = "SELECT categorie_principale, couleur FROM objets AS t1
							...
							ON t2.id_sous_categorie2 = t1.sscategorie_objet
								...
								ON t2.id_sous_categorie = t3.id_sous_categorie
									...
									ON t2.id_categorie = t4.id_categorie WHERE id_objet=" . $id_objet;
				$req = $bdd->prepare($sql);
				$req->execute();
				$result = $req->fetch(PDO::FETCH_ASSOC);
				$Couleur = $result['couleur'];
				echo "<style>.categorie_" . $id_objet . " li, .flux_commerce a, .avisenattente_commerce a {background-color:" . $result['couleur'] . " !important;}</style>\n";
			break;
			case "Utilisateur" :
				$sqlfollow = "SELECT COUNT(id_avis) AS count_follow" . $sqldroite . " WHERE (contributeurs_id_contributeur IN (SELECT contributeurs_id_contributeur FROM contributeurs_follow_contributeurs WHERE contributeurs_id_contributeurfollow = " . $id_contributeur . ")
								OR objets_id_objet IN (SELECT objets_id_objet FROM contributeurs_follow_objets WHERE contributeurs_id_contributeur = " . $id_contributeur . "))";
...
<div class="filters flux">
    <div id="PrefStat" class="avisenattente_commerce<?php if ($PAGE == "Objet") {echo " categorie_" . $id_objet;} ?>">
        <a href="#" title=""><span>Statistiques</span></a>
    </div>
    <div class="flux_commerce button_moving<?php if ($PAGE == "Objet") {echo " categorie_" . $id_objet;} ?>">
        ...
        <div class="rang0<?php if ($PAGE == "Objet") {echo " categorie_" . $id_objet;} ?>">
            <ul>
        ...
        <div class="rang1<?php if ($PAGE == "Objet") {echo " categorie_" . $id_objet;} ?>">
            <ul>
