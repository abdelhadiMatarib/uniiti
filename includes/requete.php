<?php
if (!empty($_POST['lastid']) || !empty($_POST['provenance'])) {
...
if (!empty($_POST['lastid'])) {
    $sql2 .= "WHERE date_avis < " . urldecode($_POST['lastid']);
    $ClauseWhere = true;
}
if (!empty($_POST['provenance'])) {
    if (urldecode($_POST['provenance']) == "\"avis_en_attente\"") {
        ...
        if (urldecode($_POST['provenance']) != "\"all\"") {
            $sql2 .= " AND provenance = " . urldecode($_POST['provenance']);
        }
    }
} else {
    if ($ClauseWhere) {
...
if (!empty($_POST['scategorie'])) {
    $sql2 .= " AND t10.id_sous_categorie = " . $_POST['scategorie'];
}
if (!empty($_POST['sscategorie'])) {
    $sql2 .= " AND t10.id_sous_categorie2 = " . $_POST['sscategorie'];
}
if (!empty($_POST['id_ville'])) {
    $sql2 .= " AND t9.villes_id_ville = " . $_POST['id_ville'];
}
if (!empty($_POST['id_budget'])) {
    $sql2 .= " AND t9.id_budget = " . $_POST['id_budget'];
}
/**
 * selim 
 */
if (!empty($_POST['quoi']) or !empty($_POST['lieu'])) {
    $showEnseigne = TRUE;
    $sql2 = "SELECT ";
    $sql2 .= "DISTINCT(t1.id_enseigne), t1.nom_enseigne, t1.box_enseigne, ";
    $sql2 .= "t2.sous_categorie2, ";
    $sql2 .= "t3.sous_categorie, ";
    $sql2 .= "t6.arrondissement ";
    $sql2 .= "FROM enseignes t1 ";
    $sql2 .= "INNER JOIN sous_categories2 t2 ON t1.sscategorie_enseigne = t2.id_sous_categorie2 ";
    $sql2 .= "INNER JOIN sous_categories t3 ON t2.id_sous_categorie = t3.id_sous_categorie ";
    $sql2 .= "INNER JOIN categories t4 ON t3.id_categorie = t4.id_categorie ";
    $sql2 .= "INNER JOIN villes t5 ON t1.villes_id_ville = t5.id_ville ";
    $sql2 .= "INNER JOIN quartier t7 ON t1.id_quartier = t7.id_quartier ";
    $sql2 .= "INNER JOIN arrondissement t6 ON t7.id_arrondissement = t6.id_arrondissement ";
    $sql2 .= "WHERE ";
    
    if (!empty($_POST['lieu'])) {
       $sql2 .= " t6.arrondissement LIKE '%" . addslashes(urldecode($_POST['lieu'])) . "%' ";
     }
    ...
    if (!empty($_POST['quoi'])) {
        ...
        $sql2 .= " (nom_enseigne LIKE '%" . addslashes(urldecode($_POST['quoi'])) . "%'";
...
if (!$showEnseigne) {
    ...
    $sql2 .= " ORDER BY t1.id_enseigne DESC LIMIT 0," . $NbItems;
}
...
while ($row = $req2->fetch(PDO::FETCH_ASSOC)) {
    ...
    if ($showEnseigne) {
        /*
         * debut selim
         * S'il s'agit d'une recherche sur les enseignes et qu'on trouve un seul
         * résultat => on redirige directement vers la page de l'enseigne
         * 
         */
        if ($req2->rowCount() == 1) {
            echo json_encode(array('status' => 'redirect', 'url' => "/pages/commerce.php?id_enseigne=" . $row['id_enseigne']));
            exit();
        } else {
            include '../includes/display/search_results.php';
        }
        /*
         * fin selim
         */
        ...
        if ($arrondissement == "Indéfini") {
            $arrondissement = $ville_enseigne;
        }
        ...
        <div class="box <?php echo $BoxFiltre; ?>" id="<?php echo $datetime; ?>" temps="<?php echo strtotime($datetime); ?>">
            ...
            <header>
                <div class="box_icon" title="<?php
        if ($sous_categorie2 == NULL) {
            echo $sous_categorie;
        } else {
            echo $sous_categorie2;
        }
        ?>" style="background:url('<?php echo SITE_URL; ?>/img/pictos_commerces/sprite_cat.jpg') <?php echo $scposx . "px" . " " . $scposy . "px" ?>"></div>
        <!--					<div class="box_desc" onclick="location.href='<?php echo $url; ?>/<?php echo $id_enseigne; ?>.html';">
                ...
                -->					<div class="box_desc" onclick="location.href='<?php echo $SITE_URL . "/pages/commerce.php?id_enseigne=" . $id_enseigne; ?>'">
                    ...
                    <span class="box_subtitle" title="<?php
             if ($sous_categorie2 == NULL) {
                 echo $sous_categorie;
             } else {
                 echo $sous_categorie2;
             }
        ?>" style="color:<?php echo $couleur; ?>;"><?php
                  if ($sous_categorie2 == NULL) {
                      echo $sous_categorie;
                  } else {
                      echo $sous_categorie2;
