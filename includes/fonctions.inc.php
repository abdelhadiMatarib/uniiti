...
function EcartDate($Maintenant, $Date, $hide7Days = false) {
...
function Age($Maintenant, $jour, $mois, $annee) {
    ...
    if ($jour + $mois + $annee == 0) {
        $age = "âge NC";
    } else {
        if (($mois < $an[1]) || ($mois == $an[1]) && ($jour <= $an[2])) {
            $age = $an[0] - $annee;
        } else {
            $age = $an[0] - $annee - 1;
        }
...
function AfficheEtoiles($note_arrondi, $categorie) {
    ...
    if ($reste <= -0.25) {
        $nbpleines = round($note_arrondi / 2) - 1;
    } else {
        $nbpleines = round($note_arrondi / 2);
    }
